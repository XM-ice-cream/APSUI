	...
	<div class="base-info-box">
		...
		<Card>
			...
			<el-table :data="tableData" fixed="right" :height="tableConfig.height" v-loading="loading">
				...
				<el-table-column v-for="(item, index) in Column" :prop="item.key" :key="index" :label="item.title" :width="item?.width" :min-width="item?.minWidth" :align="item?.align" :show-overflow-tooltip="item?.tooltip" :formatter="item?.formatter === 'dateFormat' ? dateFormat : null">
			...
			<!-- 分页 -->
			<PaginationCustom :total="req.total" :totalPage="req.totalPage" :pageIndex="req.pageIndex" :pageSize="req.pageSize" :elapsedMilliseconds="req.elapsedMilliseconds" @handleCurrentChange="pageChange" @handleSizeChange="pageSizeChange" />
		</Card>
	</div>
</template>

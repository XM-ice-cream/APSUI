        ...
        <el-form class="form-inline" label-width="120px" ref="submitRef"  :model="submitReq" >
            <template v-for="item in UpdateForm">
                <el-form-item :label="item.label" :prop="item.prop">
                    <el-input v-if="item.type==='input'" v-model="submitReq[item.prop]" :placeholder= "`请输入${item.label}`" />
                     <el-input-number v-if="item.type==='inputNumber'" v-model="submitReq[item.prop]" :placeholder="`请输入${item.label}`" />
                </el-form-item>
            </template>
        </el-form>
        <template #footer>
